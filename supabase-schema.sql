-- Legacy Compass Database Schema for Supabase
...
CREATE INDEX idx_user_properties_farm_name ON user_properties(farm_name);
CREATE INDEX idx_user_properties_is_hot ON user_properties(is_hot);
CREATE INDEX idx_user_properties_is_absentee ON user_properties(is_absentee);
...
COMMENT ON TABLE user_properties IS 'Stores all property data for each user with notes, tags, and status';
COMMENT ON TABLE user_settings IS 'Stores user preferences and settings';
COMMENT ON TABLE activity_log IS 'Tracks user activity and actions on properties';
